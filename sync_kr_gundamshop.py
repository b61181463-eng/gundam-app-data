...
def is_probable_product_href(href: str) -> bool:
    ...
    # 홈/목록/정렬 링크는 제외
    if lower == "https://www.gundamshop.co.kr/" or lower == "https://www.gundamshop.co.kr":
        return False

    if "/theme/reserve.html" in lower and ("cate=" in lower or "sort=" in lower or "ordr=" in lower):
        return False
    ...
    # 완화: gundamshop 내부 링크면 일단 후보 허용
    if "gundamshop.co.kr" in lower:
        return True
...
def extract_products_from_listing(url: str):
    ...
    for block in blocks:
        ...
        # 너무 빡세게 말고, 건담 키워드 또는 등급명 있으면 통과
