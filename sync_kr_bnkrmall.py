...
def verify_detail_stock(product_url: str, headers: dict) -> str:
    try:
        ...
        # 끝까지 못 찾으면 기본값은 판매중으로 두기보다
        # 상품 상세페이지가 살아있고 품절 문구가 없으면 판매중 처리
        return "판매중"
    except Exception:
        return "판매중"
...
def extract_products_from_listing(url: str):
    ...
    for block in blocks:
        ...
        if not is_allowed_domain(href, ["bnkrmall.co.kr", "www.bnkrmall.co.kr"]):
            continue
